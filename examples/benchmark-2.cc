...
{
	int iters = 5;
	printf("%% Speed test with correctness checks. Constant data size version.\n");
	...
	for (int m = min_test; m <= max_test; m+=2) {
		for (int n = min_test; n <= max_test; n+=2) {
			printf("%8i %8i ", n, m);
			for (int j = 0; j < 3; j++) {
				...
				int size = LARGE_ENOUGH / n; // Total data size will be LARGE_ENOUGH
